...
class users {
    ...
    public function save() {
        $sql = "INSERT INTO `user`(`iduser`, `username`, `user_email`, `login_status`, `last_login`) VALUES (null, :username, :email, :loginStatus, :lastLogin)";
    ...
    public function getUserByEmail() {
        $stmt = $this->dbConn->prepare('SELECT * FROM user WHERE user_email = :email');
        $stmt->bindParam(':email', $this->email);
        try {
            if($stmt->execute()) {
    ...
    public function getUserById() {
        $stmt = $this->dbConn->prepare('SELECT * FROM user WHERE iduser = :id');
    ...
    public function updateLoginStatus() {
        $stmt = $this->dbConn->prepare('UPDATE user SET login_status = :loginStatus, last_login = :lastLogin WHERE iduser = :id');
        $stmt->bindParam(':loginStatus', $this->loginStatus);
        $stmt->bindParam(':lastLogin', $this->lastLogin);
        $stmt->bindParam(':id', $this->id);
        try {
